...
interface CustomPostRepository {
    fun findPageBy(pageRequest: Pageable, postSearchRequstDto: PostSearchRequstDto): Page<Post>
}

class CustomPostRepositoryImpl : CustomPostRepository, QuerydslRepositorySupport(Post::class.java) {
    override fun findPageBy(pageRequest: Pageable, postSearchRequstDto: PostSearchRequstDto): Page<Post> {
        val result = from(post)
            .where(
                postSearchRequstDto.title?.let { post.title.contains(it) },
                postSearchRequstDto.createdBy?.let { post.createdBy.eq(it) },
