...
@Import(RestDocsConfiguration::class) // 테스트에서 별도로 구성한 config를 로딩함.
abstract class RestDocsSupport {
    ...
    fun setup(
        ...
        this.mockMvc = MockMvcBuilders.webAppContextSetup(webApplicationContext!!)
            ...
            .alwaysDo<DefaultMockMvcBuilder>(restDoc!!) // 재정의한 핸들러를 적용함. 적용하면 일반 document에도 적용됨. 일반 document로 선언되면 그부분도 같이 생성됨에 유의해야 함.
    ...
    protected fun responseCommon(): Array<FieldDescriptor> {
        return arrayOf(
            ...
            PayloadDocumentation.fieldWithPath("resultMsg").type(JsonFieldType.STRING)
                .description("결과메세지"),  //subsectionWithPath("resultData").type(JsonFieldType.OBJECT).description("결과데이터").optional()
