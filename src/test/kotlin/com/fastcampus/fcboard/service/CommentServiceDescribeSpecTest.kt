 ...
 */
@SpringBootTest
class CommentServiceDescribeSpecTest(
...
) : DescribeSpec(
    {
        ...
        describe("게시글의 댓글 생성") {
            ...
            context("게시글이 없는 댓글 생성 요청 이면") {
                it("게시글을 찾을수 없다는 에러가 발생 한다") {
        ...
        describe("댓글 변경") {
            ...
            context("정상적인 댓글 변경 요청 이면") {
                ...
                it("변경된 댓글을 확인 한다") {
                    commentResponseDto.id shouldBe savedComment.id
                    commentResponseDto.content shouldBe "수정된 댓글 내용"
                    commentResponseDto.createdBy shouldBe "댓글 작성자"
                }
            }
            context("댓글이 없는 변경 요청이 오면") {
                it("댓글을 찾을수 없다는 에러가 발생 한다") {
            ...
            context("댓글 변경 요청 사용자가 댓글 생성자가 아니면") {
                it("댓글을 변경할 수 없다는 에러를 발생 시킨다") {
        ...
        describe("댓글 삭제") {
            ...
            context("댓글이 없는 삭제 요청 이면") {
                it("댓글을 찾을 수 없다는 에러를 발생 시킨다") {
                    shouldThrow<CommentNotFoundException> {
                        commentService.deleteComment(999L, "댓글 작성자")
                    }
                }
            }
            context("댓글 삭제 요청 사용자가 댓글 생성자가 아니면") {
                it("댓글을 삭제할 수 없다는 에러를 발생 시킨다") {
                    shouldThrow<CommentNotDeletableException> {
                        commentService.deleteComment(savedComment.id, "다른 사용자")
                    }
                }
            }
            context("정상적인 댓글 삭제 요청 이면") {
